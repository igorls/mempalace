...
description: >
  ...
  what tools/sync_issues.py can do with heuristics alone. Re-reads issue
  ...
  Writes docs/TRIAGE.md. Use when the user says "triage issues", "triage
...
script `tools/sync_issues.py` does keyword classification and pattern-based
...
**Output:** `docs/TRIAGE.md` (overwrite, don't version). Keep it short and
...
Run `python tools/sync_issues.py` to regenerate `docs/ISSUES.md`. If the
file is less than 1 hour old, it's fine to skip (use `--no-cache` if you
suspect staleness). Read `docs/ISSUES.md` to get the current CRITICAL /
...
2. Read it. Decide:
   ...
   - Is there an in-flight PR addressing it? (check `docs/ISSUES.md`
...
Read the "PRs flagged for review" section of `docs/ISSUES.md`. For each
...
- What was recently merged (last 14 days — from `docs/ISSUES.md`)
...
> Generated YYYY-MM-DD from docs/ISSUES.md and live issue/PR bodies.
...
Overwrite `docs/TRIAGE.md`. Do not keep dated copies.
...
- **Cache is your friend.** Don't refetch what `docs/ISSUES.md` already
...
- **Cross-reference local `develop` branch** on the maintainer clone
  (`/home/igorls/dev/MemPalace/mempalace`) if the user wants to know
  whether a fix is already on develop vs still in a PR. Run
  `git -C /home/igorls/dev/MemPalace/mempalace log --oneline -50`
  to see recent commits.

## Common mistakes to avoid

- Regenerating `docs/ISSUES.md` every time when it's already fresh
